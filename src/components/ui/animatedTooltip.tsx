      ...
      {items.map((item, idx) => (
        <div className="-mr-4  relative group " key={item.name} onMouseEnter={() => setHoveredIndex(item.id)} onMouseLeave={() => setHoveredIndex(null)}>
          ...
          <img
            ...
            src={item.image}
            alt={item.name}
            className=" object-contain bg-white  mx-[10px] object-center p-1 rounded-full h-[60px] w-[60px] border-2 group-hover:scale-105 group-hover:z-30 border-white  relative transition duration-500"
          />
        </div>
      ))}
    </>
